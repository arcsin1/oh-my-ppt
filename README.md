...
- 🎬 **动画演示** — 支持页面切换动画，也支持基于 GSAP 的整元素动画，并保持 `data-anim` 可编辑导出语义
...
Oh My PPT 的页面是 HTML 幻灯片，内置本地 **GSAP 驱动** 的动画运行时，并通过 `data-anim` / `PPT.*` 约束对外暴露。生成或编辑页面时，可以让 AI 为标题、数据卡片、图片、图表容器、步骤模块等整块元素添加演示动画。
...
- **声明式错峰控制**：对重复元素使用 `data-anim-stagger="N"`，不再把 `stagger(N)` 塞进 delay 字符串。
- **声明式加载顺序**：使用 `data-anim-sequence="with|after"` 控制 load 顺序，不再混用 trigger 语义。
...
`gsap.*` 不是公开的创作接口。为了保证预览行为和 PPTX 导出一致，推荐始终使用 `data-anim` 或受控的 `PPT.*` 包装接口。
